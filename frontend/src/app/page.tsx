...
export default function HomePage() {
  return (
    <main className="relative min-h-screen w-full overflow-x-hidden flex flex-col bg-cover bg-top bg-no-repeat transition-all duration-500 bg-[url('/background/bg-long2.png')] dark:bg-[url('/background/bg-long-dark2.png')] font-sans">
      {/* Light atmospheric overlay for contrast security */}
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto flex flex-col px-4 sm:px-8 pt-28">
        ...
        <section className="relative flex flex-col items-center justify-center text-center w-full max-w-[640px] mx-auto mt-20 mb-6 select-none animate-fade-in">
          ...
          {/* Hero Structural Glass Framework Card */}
          <div className="w-full rounded-[28px] border border-white/15 dark:border-white/5 bg-white/[0.04] dark:bg-slate-950/[0.06] backdrop-blur-xl p-8 pt-16 flex flex-col items-center z-10 shadow-[0_8px_32px_rgba(15,23,42,0.02)]">
            ...
            {/* FIXED TYPOGRAPHY: Balanced layout widths eliminate the single-word wrap trap */}
            <p className="mx-auto mt-5 max-w-sm text-xs sm:text-sm font-sans font-medium text-slate-600/90 dark:text-slate-300/90 tracking-wide leading-relaxed">
              Cozy games for peaceful breaks<br className="hidden sm:inline" /> and late-night gaming sessions.
            </p>

            {/* Cleaned Stats Badges */}
            ...
            {/* FIXED ACCESSIBILITY BUTTONS: High-contrast theme-aware system styling tokens */}
            <div className="mt-8 flex flex-col sm:flex-row items-center gap-3.5 w-full max-w-[360px] font-sans">
              <Link href="/game/tictactoe" className="w-full">
                <button className="w-full rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-950 hover:bg-slate-800 dark:hover:bg-slate-100 py-3 text-xs font-bold uppercase tracking-wider transition-all duration-150 active:scale-[0.99] cursor-pointer shadow-sm">
                  Start Playing
                </button>
              </Link>
        ...
        {/* Featured Games Grid Layout Section */}
